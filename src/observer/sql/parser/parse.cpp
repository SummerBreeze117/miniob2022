...
void inserts_init(Inserts *inserts, const char *relation_name, InsertTuple tuples[], size_t tuple_num)
{
  ...
  for (size_t i = 0; i < tuple_num; i++) {
    assert(tuples[i].value_num <= sizeof(tuples[i].values) / sizeof(tuples[i].values[0]));
...
void inserts_destroy(Inserts *inserts)
{
  ...
  for (size_t i = 0; i < inserts->tuple_num; i++) {
    InsertTuple *tuple = &inserts->tuples[i];
    for (size_t j = 0; j < tuple->value_num; j ++)
    value_destroy(&tuple->values[j]);
  }
  inserts->tuple_num = 0;
}
...
void updates_destroy(Updates *updates)
{
  ...
  for (size_t i = 0; i < updates->attr_num; i++) {
    relation_attr_destroy(&updates->attributes[i]);
  }
  updates->attr_num = 0;
...
void create_indexs_append_attribute(CreateIndex *create_index, RelAttr *rel_attr)
{
